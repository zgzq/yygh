...
<script type="text/javascript">
	...
	function refresh() {
		...
		$("#dg").datagrid({
			url : WEB_ROOT + '/hos/surgeryDate/gridData1?'+$('#fmQuery').serialize()
		});
	...
	function exportExl(){
		...
		var url = WEB_ROOT + '/hos/surgeryDate/export?month='+$("#month").val()+"&year="+
		$("#year").val()+"&departid="+$("#departid").val()+"&month2="+$("#month2").val();
		window.open(url);
	}
	function reset(){
		$.ajax({
            ...
            type: "Post",
            //方法所在页面和方法名   
            url: WEB_ROOT + '/hos/surgeryDate/geneData?departid='+ $("#departid").val()
			+ "&year=" + $("#year").val()+"&month="+ $("#month").val()+"&month2="+ $("#month2").val(),
            data: '',
	...
	<div id="tb" style="padding: 0px; height: auto">
		...
		<form id="fmQuery" method="post">
			<div
				...
				<select class="easyui-combobox" name="month" id="month2"
					style="width: 10%;">
					<option value=""></option>
					<option value="01">1月</option>
					<option value="02">2月</option>
