...
@Menu("中美医院综合")
@RequestMapping("/kpi/zm")
@Controller
public class ZmKpiController extends AuthController {
	private static Logger logger = org.slf4j.LoggerFactory
			.getLogger(ZmKpiController.class);

	@Action("中美医院综合")
	@RequestMapping("/list")
	public String index(Model m) {
		List<KpiValueMonthV> kpiValueMonthV=KpiValueMonthV.INSTANCE.query("planid=? and orgid=?", 11,1);
		List<String> listMonth=new ArrayList<String>();
		for (KpiValueMonthV kvc : kpiValueMonthV) {
			listMonth.add(kvc.getMonth());
		}
		m.addAttribute("month", listMonth);
		return "admin/zmrpt/list";
	}
...
//			}
	...
	public String download() {
		...
		String filename="中美综合指标"+sdate+"-"+edate;
