...
@Menu("誉美医院综合")
@RequestMapping("/kpi/ym")
@Controller
public class YmKpiController extends AuthController {

	@Action("誉美医院综合")
	@RequestMapping("/list")
	public String index(Model m) {
		List<KpiValueMonthV> kpiValueMonthV=KpiValueMonthV.INSTANCE.query("planid=? and orgid=?", 12,2);
		List<String> listMonth=new ArrayList<String>();
		for (KpiValueMonthV kvc : kpiValueMonthV) {
			listMonth.add(kvc.getMonth());
		}
		m.addAttribute("month", listMonth);
		return "admin/ymrpt/list";
	}
...
//			}
	...
	public String download() {
		...
		String filename="誉美综合指标"+sdate+"-"+edate;
